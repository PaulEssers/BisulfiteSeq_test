...
for file in $(ls *.fastq); do echo ${file::(-8)}
...
sbatch -p blade,himem,hugemem --cpus-per-task=2 -o ../slurm_logs/bismark_${file::(-8)}.%j.out ${tmp}bismark_${file::(-8)}.sh 2>&1 | tee ${tmp}bismark_${file::(-8)}.id
